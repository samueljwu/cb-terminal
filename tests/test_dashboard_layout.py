...
class DashboardLayoutTests(unittest.TestCase):
    def test_instrument_command_bar_and_assumptions_tab_drive_pm_layout(self):
        ...
        self.assertIn('border-radius:clamp(10px, 1vw, 14px)', html)
        self.assertIn('min-height:clamp(46px, 4vw, 54px)', html)
        ...
        self.assertGreaterEqual(html.count('font-size:var(--text-command)'), 2)
    ...
    def test_plots_are_compact_enough_to_reduce_scroll_capture(self):
        html = render_dashboard_html()
        self.assertIn('svg.matlab-plot { width:100%; height:clamp(220px, 24vw, 300px); min-height:0;', html)
        self.assertIn('svg.matlab-plot.small-plot { height:clamp(175px, 18vw, 225px); }', html)
        self.assertIn('svg.matlab-plot.pm-plot { height:clamp(210px, 21vw, 270px); }', html)
        self.assertIn('svg.matlab-plot.pm-small-plot { height:clamp(165px, 16vw, 210px); }', html)
    ...
    def test_prospectus_extract_selected_uses_raw_pdf_selection_and_data_upload_owns_upload(self):
        ...
        self.assertIn('normalized internally into long-form records; the source workbook stays unchanged', html)
        self.assertIn("payload.input_normalization?.status === 'normalized'", html)
        self.assertIn('Source file preserved.', html)
        self.assertIn("(payload.warnings || []).slice(-3)", html)
    ...
    def test_assumption_action_buttons_use_compact_command_group_style(self):
        ...
        self.assertIn('.intake-action-row button { padding:6px 9px; min-height:28px; font-size:var(--text-sm);', html)
    ...
    def test_layout_uses_adaptive_type_grids_and_narrow_screen_containment(self):
        ...
        self.assertIn('grid-template-columns:repeat(auto-fit,minmax(min(150px,100%),1fr))', html)
        self.assertIn('grid-template-columns:repeat(auto-fit,minmax(min(250px,100%),1fr))', html)
        self.assertIn('grid-template-columns:repeat(auto-fit,minmax(min(300px,100%),1fr))', html)
        self.assertIn('.tab-bar { display:flex; gap:6px; flex-wrap:nowrap;', html)
        ...
        self.assertIn('.term-table td:nth-child(3)::before { content:"Evidence"; }', html)
        self.assertIn('pre { max-width:100%; white-space:pre-wrap; overflow-wrap:anywhere;', html)
        self.assertIn('class="workflow-box workflow-overview"', html)
        self.assertIn('class="workflow-graph-scroll"', html)
